...
?><? if (strlen($tag["attributes"]["file"])>0) { $event = explode("/", $tag["attributes"]["file"]); ?>
	...
	<? if (!file_exists($path)) { 
		...
		$tbl = new Database("tbl_prailsbase_");
	...
	<@=OutputGenerator::getInstance()->includeTemplate("<?=$path?>", $arr_param)@>
<? } else if (strlen($tag["attributes"]["event"])>0) { ?>
	<@=invoke("<?=$tag['attributes']['event']?>", $arr_param)@>
<? } else if (strlen($tag['attributes']['template']) > 0) { ?>
	<@=OutputGenerator::getInstance()->includeTemplate("<?=str_replace('.html', '.'.$tag['attributes']['template'].".html", $this->template)?>", $arr_param);@>
<? } ?>
