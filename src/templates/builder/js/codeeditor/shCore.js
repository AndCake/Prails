...
(function() {
	...
	var c = /\$(?:(\d\d?|[$&`'])|{([$\w]+)})/g, h = /[^gimy]+|([\s\S])(?=[\s\S]*\1)/g, n = /^(?:[?*+]|{\d+(?:,\d*)?})\??/, g = false, k = [], m = {
		exec : RegExp.prototype.exec,
		test : RegExp.prototype.test,
		match : String.prototype.match,
		replace : String.prototype.replace,
		split : String.prototype.split
	}, a = m.exec.call(/()??/, "")[1] === undefined, e = function() {
		var p = /^/g;
		m.test.call(p, "");
		return !p.lastIndex
	}(), f = function() {
	...
	String.prototype.replace = function(r, s) {
		...
		if (typeof s === "function") {
			p = m.replace.call(this, r, function() {
				...
				if (t && r.global) {
					r.lastIndex = arguments[arguments.length - 2]
							+ arguments[0].length
				}
				...
				return m.replace.call(s, c, function(x, w, A) {
					if (w) {
						...
						case "&":
							return v[0];
						case "`":
							return v[v.length - 1].slice(0, v[v.length - 2]);
						case "'":
							return v[v.length - 1].slice(v[v.length - 2]
									+ v[0].length);
						default:
							var y = "";
							w = +w;
	...
	function d(p) {
		return (p.global ? "g" : "") + (p.ignoreCase ? "i" : "")
				+ (p.multiline ? "m" : "") + (p.extended ? "x" : "")
				+ (p.sticky ? "y" : "")
	}
	function o(v, u, w, p) {
	...
	XRegExp.addToken(/\\k<([\w$]+)>/, function(q) {
		...
		return p > -1 ? "\\" + (p + 1)
				+ (isNaN(q.input.charAt(q.index + q[0].length)) ? "" : "(?:)")
				: q[0]
	});
	XRegExp.addToken(/\[\^?]/, function(p) {
		return p[0] === "[]" ? "\\b\\B" : "[\\s\\S]"
...
function removeHighlighter(highlighter) {
	sh.vars.highlighters[getHighlighterById(highlighter.id)] = null;
}
...
sh.Highlighter.prototype = {
	 ...
	 */
	getCodeLinesHtml: function(html, lineNumbers, singleLine)
	{
		html = trim(html);
