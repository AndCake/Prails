...
var XMLParser = Editor.Parser = (function() {
    ...
    function inText(source, setState) {
      ...
      if (ch == "<") {
        if (source.equals("!")) {
		  ...
		  // read the processing instruction, e.g. <?php
		  source.next();
		  source.nextWhile(matcher(/[\w\d_]/));
          setState(inBlock("xml-processing", "?>"));
          return "xml-processing-start";
        }
        else {
  ...
  function parseXML(source) {
    ...
    // Add a lyer of style to the current token's, for example syntax-error
    function mark_add(current, style){
    ...
    function expect(text) {
      return function(style, content) {
        ...
        else mark_add(style, "xml-error") || cont(arguments.callee);
      };
    }
    ...
    var harmlessTokens = {"xml-text": true, "xml-entity": true, "xml-comment": true,
                          "xml-cdata": true, "xml-processing": true, "xml-processing-start": true};
    function element(style, content) {
      if (content == "<") cont(tagname, attributes, endtag(tokenNr == 1));
      else if (content == "</") cont(closetagname, expect(">"));
      // FIXME: need example of when this is encountered... it seems to handle xml-processing, although...
      else if (content == "<?") cont(tagname, attributes, expect("?>"));
      // FIXME: ...harmlessTokens.hasOwnProperty("xml-processing") is true, but would never be reached
      else if (harmlessTokens.hasOwnProperty(style)) cont();
      else mark_add(style, "xml-error") || cont();
    }
    ...
    function closetagname(style, content) {
      if (style == "xml-name" && context && content.toLowerCase() == context.name) {
        ...
        mark_add("xml-tagname", "xml-error");  // a tagname was expected here
      }
      cont();
    }
