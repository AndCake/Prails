			...
			.apply(
					Ext,
					{
						...
						num : function(v, defaultValue) {
							v = Number(v == null || typeof v == 'boolean' ? NaN
									: v);
						...
						isMac : isMac,
						isAir : isAir,
						useShims : ((isIE && !(isIE7 || isIE8)) || (isMac
								&& isGecko && !isGecko3))
					});
...
( function() {
	...
	Ext.lib.Dom = {
		...
		getViewportWidth : function() {
			...
			var mode = document.compatMode;
			if (mode || Ext.isIE) {
				width = (mode == "CSS1Compat") ? document.documentElement.clientWidth
...
Ext.EventManager = function() {
	...
	var propRe = /^(?:scope|delay|buffer|single|stopEvent|preventDefault|stopPropagation|normalized|args|delegate)$/, curWidth = 0, curHeight = 0;
	var pub = {
	...
	Ext.EventObjectImpl.prototype = {
		...
		isSpecialKey : function() {
			...
			return (this.type == 'keypress' && this.ctrlKey)
					|| this.isNavKeyPress() || (k == this.BACKSPACE)
					|| (k >= 16 && k <= 20) || (k >= 44 && k <= 45);
		},
		...
		getTarget : function(selector, maxDepth, returnEl) {
			return selector ? Ext.fly(this.target).findParent(selector,
					maxDepth, returnEl) : (returnEl ? Ext.get(this.target)
					: this.target);
		},
		...
		within : function(el, related, allowEl) {
			...
			return t
					&& ((allowEl ? (t === Ext.getDom(el)) : false) || Ext.fly(
							el).contains(t));
		},
		getPoint : function() {
...
Ext.Fx = {
	slideIn : function(anchor, o) {
		...
		el.queueFx(o, function() {
			...
			wrap.show();
			arguments.callee.anim = wrap.fxanim(a, o, 'motion', .5, 'easeOut',
					after);
		});
		return this;
	},
	...
	highlight : function(color, o) {
		...
		el.queueFx(o, function() {
			...
			arguments.callee.anim = this.fxanim(a, o, 'color', 1, 'easeIn',
					after);
		});
		return this;
	},
	frame : function(color, count, o) {
		var el = this.getFxEl(), proxy, active;
		o = o || {};
		el.queueFx(o, function() {
			color = color || "#C3DAF9"
			if (color.length == 6) {
			...
			count = count || 1;
			this.show();
			var xy = this.getXY(), dom = this.dom, b = {
				x : xy[0],
				y : xy[1],
				0 : xy[0],
				1 : xy[1],
			...
			}, proxy, queue = function() {
				proxy = Ext.get(document.body || document.documentElement)
						.createChild( {
							style : {
								visbility : 'hidden',
								position : 'absolute',
	...
	ghost : function(anchor, o) {
		...
		el.queueFx(o, function() {
			...
			arguments.callee.anim = this.fxanim(a, o, 'motion', .5, "easeOut",
					after);
		});
		return this;
	},
...
Ext.extend(Ext.CompositeElementLite, Ext.CompositeElement, {
	...
	addListener : function(eventName, handler, scope, opt) {
		var els = this.elements;
		for ( var i = 0, len = els.length; i < len; i++) {
			Ext.EventManager.on(els[i], eventName, handler, scope || els[i],
					opt);
		}
		return this;
	},
				...
				Ext.util.Observable,
				{
					...
					request : function(o) {
						if (this.fireEvent("beforerequest", this, o) !== false) {
							...
							if ((method == 'GET' || o.xmlData || o.jsonData)
									&& p) {
								url += (url.indexOf('?') != -1 ? '&' : '?') + p;
								p = '';
							}
							this.transId = Ext.lib.Ajax.request(method, url,
									cb, p, o);
							return this.transId;
						} else {
							Ext
									.callback(o.callback, o.scope, [ o, null,
											null ]);
							return null;
						}
					},
					...
					doFormUpload : function(o, ps, url) {
						...
						var form = Ext.getDom(o.form), buf = {
							target : form.target,
							method : form.method,
						...
						function cb() {
							...
							Ext.EventManager.removeListener(frame, 'load', cb,
									this);
...
Ext.Updater = Ext.extend(Ext.util.Observable, {
	...
	update : function(url, params, callback, discardUrl) {
		if (this.fireEvent("beforeupdate", this.el, url, params) !== false) {
			...
			var o = Ext.apply( {}, {
				url : url,
				params : (typeof params == "function" && callerScope) ? params
						.createDelegate(callerScope) : params,
	...
	startAutoRefresh : function(interval, url, params, callback, refreshNow) {
		...
		this.autoRefreshProcId = setInterval(this.update.createDelegate(this, [
				url || this.defaultUrl, params, callback, true ]),
				interval * 1000);
	},
	...
	processSuccess : function(response) {
		...
		if (this.loadScripts) {
			this.renderer.render(this.el, response, this, this.updateComplete
	...
	updateComplete : function(response) {
		...
		if (typeof response.argument.callback == "function") {
			response.argument.callback.call(response.argument.scope, this.el,
					true, response, response.argument.options);
		}
	},
	processFailure : function(response) {
		...
		if (typeof response.argument.callback == "function") {
			response.argument.callback.call(response.argument.scope, this.el,
					false, response, response.argument.options);
		}
	},
					...
					Date,
					{
						...
						parseDate : function(input, format, strict) {
							...
							return p[format](input,
									strict === undefined ? Date.useStrict
											: strict);
						},
						...
						createFormat : function(format) {
							...
							Date.formatFunctions[format] = new Function(
									"return " + code.join('+'));
						},
						createParser : function() {
							...
							return function(format) {
								var regexNum = Date.parseRegexes.length, currentGroup = 1, calc = [], regex = [], special = false, ch = "";
								for ( var i = 0; i < format.length; ++i) {
									ch = format.charAt(i);
									if (!special && ch == "\\") {
										special = true;
		...
		.apply(
				Date.prototype,
				{
					...
					getDayOfYear : function() {
						var i = 0, num = 0, d = this.clone(), m = this
								.getMonth();
					...
					getWeekOfYear : function() {
						...
						return function() {
							var DC3 = Date.UTC(this.getFullYear(), this
									.getMonth(), this.getDate() + 3)
									/ ms1d, AWN = Math.floor(DC3 / 7), Wyr = new Date(
									AWN * ms7d).getUTCFullYear();
							return AWN - Math.floor(Date.UTC(Wyr, 0, 7) / ms7d)
									+ 1;
						}
					}(),
					isLeapYear : function() {
					...
					clearTime : function(clone) {
						...
						if (this.getDate() != d) {
							for ( var hr = 1, c = this.add(Date.HOUR, hr); c
									.getDate() != d; hr++, c = this.add(
									Date.HOUR, hr))
								;
							this.setDate(d);
					...
					add : function(interval, value) {
						...
						case Date.MONTH:
							...
							if (day > 28) {
								day = Math.min(day, this.getFirstDateOfMonth()
										.add('mo', value).getLastDateOfMonth()
										.getDate());
							}
							d.setDate(day);
			...
			.apply(
					Date.prototype,
					{
						...
						setMonth : function(num) {
							if (num <= -1) {
								var n = Math.ceil(-num), back_year = Math
										.ceil(n / 12), month = (n % 12) ? 12 - n % 12
										: 0;
								this
										.setFullYear(this.getFullYear()
												- back_year);
...
Ext.extend(Ext.util.MixedCollection, Ext.util.Observable, {
	...
	item : function(key) {
		var mk = this.map[key], item = mk !== undefined ? mk
				: (typeof key == 'number') ? this.items[key] : undefined;
	...
	getRange : function(start, end) {
		...
		start = start || 0;
		end = Math.min(typeof end == "undefined" ? this.length - 1 : end,
				this.length - 1);
		var r = [];
		if (start <= end) {
		...
		if (m3) {
			...
			if (exp) {
				fn = new Function('values', 'parent', 'xindex', 'xcount',
						'with(values){ return ' + (Ext.util.Format
								.htmlDecode(exp)) + '; }');
			}
		}
		if (m4) {
...
Ext
		.extend(
				...
				Ext.Template,
				{
					re : /\{([\w-\.\#]+)(?:\:([\w\.]*)(?:\((.*?)?\))?)?(\s?[\+\-\*\\]\s?[\d\.\+\-\*\\\(\)]+)?\}/g,
					codeRe : /\{\[((?:\\\]|.|\n)*?)\]\}/g,
					applySubTemplate : function(id, values, parent, xindex,
							xcount) {
						var t = this.tpls[id];
						if (t.test
								&& !t.test.call(this, values, parent, xindex,
										xcount)) {
							return '';
						}
						if (t.exec
								&& t.exec.call(this, values, parent, xindex,
										xcount)) {
							return '';
						}
						var vs = t.target ? t.target.call(this, values, parent)
								: values;
						parent = t.target ? values : parent;
						if (t.target && Ext.isArray(vs)) {
							var buf = [];
							for ( var i = 0, len = vs.length; i < len; i++) {
								buf[buf.length] = t.compiled.call(this, vs[i],
										parent, i + 1, len);
							}
							return buf.join('');
						}
						return t.compiled
								.call(this, vs, parent, xindex, xcount);
					},
					...
					applyTemplate : function(values) {
						return this.master.compiled
								.call(this, values, {}, 1, 1);
					},
					compile : function() {
						return this;
					}
...
Ext.KeyNav.prototype = {
	...
	isKeyDown : function() {
		return this.forceKeyDown || Ext.isIE
				|| (Ext.isWebKit && !Ext.isSafari2) || Ext.isAir;
	}
};
...
( function() {
	...
	Ext.dd.DragDrop.prototype = {
		...
		constrainTo : function(constrainTo, pad, inContent) {
			...
			this.setXConstraint(leftSpace - (pad.left || 0), c.width
					- leftSpace - b.width - (pad.right || 0), this.xTickSize);
			this.setYConstraint(topSpace - (pad.top || 0), c.height - topSpace
					- b.height - (pad.bottom || 0), this.yTickSize);
		},
		getEl : function() {
			if (!this._domRef) {
...
Ext.dd.ScrollManager = function() {
	...
	var doScroll = function() {
		if (ddm.dragCurrent) {
			...
			var inc = proc.el.ddScrollConfig ? proc.el.ddScrollConfig.increment
					: dds.increment;
			if (!dds.animate) {
	...
	var startProc = function(el, dir) {
		...
		proc.el = el;
		proc.dir = dir;
		var freq = (el.ddScrollConfig && el.ddScrollConfig.frequency) ? el.ddScrollConfig.frequency
...
Ext
		.extend(Ext.dd.DragSource, Ext.dd.DDProxy,
				{
					dropAllowed : "x-dd-drop-ok",
					dropNotAllowed : "x-dd-drop-nodrop",
					...
					onDragEnter : function(e, id) {
						...
						if (this.beforeDragEnter(target, e, id) !== false) {
							if (target.isNotifyTarget) {
								var status = target.notifyEnter(this, e,
										this.dragData);
					...
					onDragOver : function(e, id) {
						var target = this.cachedTarget
								|| Ext.dd.DragDropMgr.getDDById(id);
						if (this.beforeDragOver(target, e, id) !== false) {
							if (target.isNotifyTarget) {
								var status = target.notifyOver(this, e,
										this.dragData);
								this.proxy.setStatus(status);
							}
...
Ext.extend(Ext.dd.DropZone, Ext.dd.DropTarget, {
	...
	notifyDrop : function(dd, e, data) {
		...
		var n = this.getTargetFromEvent(e);
		return n ? this.onNodeDrop(n, dd, e, data) : this.onContainerDrop(dd,
				e, data);
	},
	triggerCacheRefresh : function() {
		Ext.dd.DDM.refreshCache(this.groups);
...
Ext.extend(Ext.data.Store, Ext.util.Observable, {
	...
	collect : function(dataIndex, allowNull, bypassFilter) {
		var d = (bypassFilter === true && this.snapshot) ? this.snapshot.items
				: this.data.items;
		var v, sv, r = [], l = {};
...
Ext.extend(Ext.data.ScriptTagProxy, Ext.data.DataProxy, {
	...
	load : function(params, reader, callback, scope, arg) {
		if (this.fireEvent("beforeload", this, params) !== false) {
			...
			trans.timeoutId = this.handleFailure.defer(this.timeout, this,
					[ trans ]);
					...
					readRecords : function(o) {
						...
						for ( var i = 0; i < c; i++) {
							...
							for ( var j = 0; j < fl; j++) {
								f = fi[j];
								var v = this.ef[j](n);
								values[f.name] = f.convert(
										(v !== undefined) ? v : f.defaultValue,
										n);
							}
	...
	readRecords : function(doc) {
		...
		for ( var i = 0, len = ns.length; i < len; i++) {
			...
			for ( var j = 0, jlen = fields.length; j < jlen; j++) {
				...
				var v = q.selectValue(Ext.isEmpty(f.mapping, true) ? f.name
						: f.mapping, n, f.defaultValue);
				v = f.convert(v, n);
				values[f.name] = v;
			}
				...
				Ext.util.Observable,
				{
					...
					appendChild : function(node) {
						...
						if (multi) {
							...
							if (oldParent) {
								if (node.fireEvent("beforemove", node
										.getOwnerTree(), node, oldParent, this,
										index) === false) {
									return false;
								}
							...
							this.fireEvent("append", this.ownerTree, this,
									node, index);
							if (oldParent) {
								node.fireEvent("move", this.ownerTree, node,
					...
					insertBefore : function(node, refNode) {
						...
						var refIndex = index;
						if (oldParent == this
								&& this.childNodes.indexOf(node) < index) {
							refIndex--;
						}
						if (oldParent) {
							if (node.fireEvent("beforemove", node
						...
						node.setOwnerTree(this.getOwnerTree());
						this.fireEvent("insert", this.ownerTree, this, node,
								refNode);
						if (oldParent) {
							node.fireEvent("move", this.ownerTree, node,
									oldParent, this, refIndex, refNode);
						}
						return node;
					},
					remove : function() {
				...
				Ext.util.Observable,
				{
							...
							if (id) {
								...
								if (state) {
									if (this.fireEvent('beforestaterestore',
										...
										this.fireEvent('staterestore', this,
												state);
									}
								}
							}
					...
					saveState : function() {
						if (Ext.state.Manager && this.stateful !== false) {
							...
							if (id) {
								...
								if (this.fireEvent('beforestatesave', this,
										state) !== false) {
					...
					removeClass : function(cls) {
						if (this.el) {
							...
							this.cls = this.cls.split(' ').remove(cls)
									.join(' ');
						}
					},
					...
					getAutoCreate : function() {
						var cfg = typeof this.autoCreate == "object" ? this.autoCreate
					...
					focus : function(selectText, delay) {
						if (delay) {
							this.focus.defer(typeof delay == 'number' ? delay
									: 10, this, [ selectText, false ]);
							return;
						}
						if (this.rendered) {
							this.el.focus();
					...
					onShow : function() {
						if (this.hideParent) {
							this.container
									.removeClass('x-hide-' + this.hideMode);
						} else {
					...
					findParentByType : function(xtype) {
						return typeof xtype == 'function' ? this
								.findParentBy( function(p) {
					...
					Ext.Element,
					{
						...
						showAction : function() {
							...
							if (this.useDisplay === true) {
								...
								supr.setLeftTop.call(this, this.lastLT[0],
										this.lastLT[1]);
							}
						},
						hideAction : function() {
						...
						setBounds : function(x, y, w, h, a, d, c, e) {
							...
							if (!a) {
								...
								supr.setBounds.call(this, x, y, w, h, a, d, cb,
										e);
							}
							return this;
						},
...
Ext.Shadow.Pool = function() {
	var p = [];
	var markup = Ext.isIE ? '<div class="x-ie-shadow"></div>'
	...
	onEndProxyDrag : function(e) {
		...
		if (this.orientation == Ext.SplitBar.HORIZONTAL) {
			newSize = this.dragSpecs.startSize
					+ (this.placement == Ext.SplitBar.LEFT ? endPoint[0]
							- this.dragSpecs.startPoint[0]
							: this.dragSpecs.startPoint[0] - endPoint[0]);
		} else {
			newSize = this.dragSpecs.startSize
					+ (this.placement == Ext.SplitBar.TOP ? endPoint[1]
...
Ext.layout.FitLayout = Ext
		.extend(
				Ext.layout.ContainerLayout,
				{
					monitorResize : true,
					onLayout : function(ct, target) {
						Ext.layout.FitLayout.superclass.onLayout.call(this, ct,
								target);
						if (!this.container.collapsed) {
							var sz = (Ext.isIE6 && Ext.isStrict && target.dom == document.body) ? target
									.getViewSize()
									: target.getStyleSize();
							this.setItemSize(this.activeItem
									|| ct.items.itemAt(0), sz);
						}
					},
...
Ext.layout.AnchorLayout = Ext.extend(Ext.layout.ContainerLayout, {
	...
	getAnchorViewSize : function(ct, target) {
		return target.dom == document.body ? target.getViewSize() : target
				.getStyleSize();
	},
	onLayout : function(ct, target) {
		...
		for (i = 0; i < len; i++) {
			...
			if (c.anchor) {
				...
				cw = a.right ? this.adjustWidthAnchor(a.right(w), c)
						: undefined;
				ch = a.bottom ? this.adjustHeightAnchor(a.bottom(h), c)
						: undefined;
				if (cw || ch) {
				...
				Ext.layout.ContainerLayout,
				{
					...
					onLayout : function(ct, target) {
						...
						var w = size.width - target.getPadding('lr')
								- this.scrollOffset, h = size.height
								- target.getPadding('tb'), pw = w;
						this.innerCt.setWidth(w);
						for (i = 0; i < len; i++) {
							c = cs[i];
							if (!c.columnWidth) {
								pw -= (c.getSize().width + c.getEl()
										.getMargins('lr'));
							}
						}
						pw = pw < 0 ? 0 : pw;
					...
					onLayout : function(ct, target) {
						...
						if (!this.rendered) {
							...
							for ( var i = 0, len = items.length; i < len; i++) {
								...
								c.collapsed = false;
								if (!c.rendered) {
									c.cls = c.cls ? c.cls + ' x-border-panel'
											: 'x-border-panel';
									c.render(target, i);
								}
								this[pos] = pos != 'center' && c.split ? new Ext.layout.BorderLayout.SplitRegion(
	...
	if (this.collapsible) {
		...
		if (this.collapseMode == 'mini' && !this.cmargins) {
			...
			this.cmargins = Ext.applyIf(this.cmargins || {}, pos == 'north'
					|| pos == 'south' ? this.defaultNSCMargins
		...
		.extend(
				...
				Ext.layout.BorderLayout.Region,
				{
					...
					render : function(ct, p) {
						...
						this.split.getMaximumSize = this[s.maxFn]
								.createDelegate(this);
...
Ext.layout.FormLayout = Ext
		.extend(
				Ext.layout.AnchorLayout,
				{
					...
					setContainer : function(ct) {
						...
						if (ct.hideLabels) {
							...
							if (typeof ct.labelWidth == 'number') {
								var pad = (typeof ct.labelPad == 'number' ? ct.labelPad
										: 5);
								this.labelAdjust = ct.labelWidth + pad;
								this.labelStyle = "width:" + ct.labelWidth
										+ "px;";
					...
					adjustWidthAnchor : function(value, comp) {
						return value
								- (comp.isFormField ? (comp.hideLabel ? 0
										: this.labelAdjust) : 0);
					},
					isValidParent : function(c, target) {
						return true;
...
Ext.layout.TableLayout = Ext
		.extend(
				Ext.layout.ContainerLayout,
				{
					monitorResize : false,
					setContainer : function(ct) {
						Ext.layout.TableLayout.superclass.setContainer.call(
								this, ct);
						this.currentRow = 0;
					...
					getNextCell : function(c) {
						...
						for ( var rowIndex = curRow; rowIndex < curRow
								+ (c.rowspan || 1); rowIndex++) {
							if (!this.cells[rowIndex]) {
								this.cells[rowIndex] = [];
							}
							for ( var colIndex = curCol; colIndex < curCol
									+ (c.colspan || 1); colIndex++) {
		...
		.extend(
				Ext.Container,
				{
					...
					createElement : function(name, pnode) {
						...
						if (name === 'bwrap'
								|| this.elements.indexOf(name) != -1) {
							if (this[name + 'Cfg']) {
								this[name] = Ext.fly(pnode).createChild(
										this[name + 'Cfg']);
							} else {
					...
					onRender : function(ct, position) {
						...
						if (this.frame) {
							...
							this.createElement('footer',
									bw.lastChild.firstChild.firstChild);
							if (!this.footer) {
						...
						if (this.border === false) {
							...
							if (this.header) {
								this.header
										.addClass(this.headerCls + '-noborder');
							}
							if (this.footer) {
								this.footer
										.addClass(this.footerCls + '-noborder');
							}
							if (this.tbar) {
						...
						if (this.rendered && this.header) {
							if (this.frame) {
								...
								var img = hd.firstChild
										&& String(hd.firstChild.tagName)
												.toLowerCase() == 'img' ? hd.firstChild
										: null;
								if (img) {
									Ext.fly(img)
											.replaceClass(old, this.iconCls);
								} else {
									Ext.DomHelper
											.insertBefore(
													hd.firstChild,
						...
						for ( var i = 0, a = arguments, len = a.length; i < len; i++) {
							...
							if (!this.tools[tc.id]) {
								...
								var t = this.toolTemplate
										.insertFirst(
												(tc.align !== 'left') ? this[this.toolTarget]
														: this[this.toolTarget]
																.child('span'),
												tc, true);
								this.tools[tc.id] = t;
								t.enableDisplayMode('block');
								t.on('click', this.createToolHandler(t, tc,
										overCls, this));
								if (tc.on) {
									t.on(tc.on);
								}
					...
					onExpand : function(doAnim, animArg) {
						if (doAnim) {
							this[this.collapseEl].slideIn(this.slideAnchor, Ext
									.apply(this.createEffect(animArg || true,
					...
					getFrameWidth : function() {
						...
						if (this.frame) {
							...
							w += (Ext.fly(l).getFrameWidth('l') + Ext.fly(
									l.firstChild).getFrameWidth('r'));
					...
					createGhost : function(cls, useShim, appendTo) {
						...
						if (this.header) {
							el.appendChild(this.el.dom.firstChild
									.cloneNode(true));
						}
					...
					doAutoLoad : function() {
						...
						u
								.update(typeof this.autoLoad == 'object' ? this.autoLoad
										: {
											url : this.autoLoad
										});
					},
...
Ext.extend(Ext.state.Provider, Ext.util.Observable, {
	get : function(name, defaultValue) {
		return typeof this.state[name] == "undefined" ? defaultValue
				: this.state[name];
	},
	clear : function(name) {
				...
				Ext.BoxComponent,
				{
					...
					refresh : function() {
						...
						if (records.length < 1) {
							...
							this.tpl.overwrite(this.el, this.collectData(
									records, 0));
							this.all.fill(Ext.query(this.itemSelector,
									this.el.dom));
					...
					collectData : function(records, startIndex) {
						var r = [];
						for ( var i = 0, len = records.length; i < len; i++) {
							r[r.length] = this.prepareData(records[i].data,
									startIndex + i, records[i]);
						}
						return r;
					},
					...
					onAdd : function(ds, records, index) {
						...
						if (index < this.all.getCount()) {
							n = this.all.item(index).insertSibling(nodes,
									'before', true);
							a.splice.apply(a, [ index, 0 ].concat(nodes));
						} else {
							n = this.all.last().insertSibling(nodes, 'after',
									true);
					...
					findItemFromChild : function(node) {
						return Ext.fly(node).findParent(this.itemSelector,
								this.el);
					},
					onClick : function(e) {
					...
					onMouseOver : function(e) {
						...
						if (item && item !== this.lastItem) {
							...
							this.fireEvent("mouseenter", this, this
									.indexOf(item), item, e);
						}
					},
					onMouseOut : function(e) {
						if (this.lastItem) {
							if (!e.within(this.lastItem, true, true)) {
								Ext.fly(this.lastItem).removeClass(
										this.overClass);
					...
					doMultiSelection : function(item, index, e) {
						if (e.shiftKey && this.last !== false) {
							...
							if ((e.ctrlKey || this.simpleSelect)
					...
					getNodes : function(start, end) {
						var ns = this.all.elements;
						start = start || 0;
						end = typeof end == "undefined" ? Math.max(
								ns.length - 1, 0) : end;
						var nodes = [], i;
						if (start <= end) {
					...
					onBeforeLoad : function() {
						if (this.loadingText) {
							this.clearSelections(false, true);
							this.el
									.update('<div class="loading-indicator">' + this.loadingText + '</div>');
							this.all.clear();
						}
					},
					onDestroy : function() {
				...
				Ext.Component,
				{
					...
					select : function(color) {
						...
						if (color != this.value || this.allowReselect) {
							...
							el.child("a.color-" + color).addClass(
									"x-color-palette-sel");
							this.value = color;
		...
		.extend(
				Ext.Component,
				{
					...
					doDisabled : function(disabled) {
						...
						if (this.showToday) {
							this.todayKeyListener[disabled ? 'disable'
									: 'enable'](disabled);
...
Ext.TabPanel = Ext
		.extend(
				Ext.Panel,
				{
					...
					onRender : function(ct, position) {
						Ext.TabPanel.superclass.onRender.call(this, ct,
								position);
						if (this.plain) {
							var pos = this.tabPosition == 'top' ? 'header'
									: 'footer';
						...
						var beforeEl = (this.tabPosition == 'bottom' ? this.stripWrap
								: null);
						...
						this.body
								.addClass('x-tab-panel-body-' + this.tabPosition);
						if (!this.itemTpl) {
							var tt = new Ext.Template(
					...
					findTargets : function(e) {
						...
						if (itemEl) {
							item = this.getComponent(itemEl.id
					...
					initTab : function(item, index) {
						...
						var el = before ? this.itemTpl.insertBefore(before, p)
					...
					onBeforeAdd : function(item) {
						var existing = item.events ? (this.items
								.containsKey(item.getItemId()) ? item : null)
								: this.items.get(item);
						if (existing) {
							this.setActiveTab(item);
							return false;
					...
					onItemIconChanged : function(item, iconCls, oldCls) {
						var el = this.getTabEl(item);
						if (el) {
							Ext.fly(el).child('span.x-tab-strip-text')
									.replaceClass(oldCls, iconCls);
						}
					},
					...
					autoSizeTabs : function() {
						...
						var ce = this.tabPosition != 'bottom' ? 'header'
								: 'footer';
						...
						var each = Math.max(Math.min(Math.floor((aw - 4)
								/ count)
					...
					scrollToTab : function(item, animate) {
						...
						var el = this.getTabEl(item);
						var pos = this.getScrollPos(), area = this
								.getScrollArea();
						var left = Ext.fly(el).getOffsetsTo(this.stripWrap)[0]
								+ pos;
					...
					scrollTo : function(pos, animate) {
						this.stripWrap.scrollTo('left', pos, animate ? this
								.getScrollAnim() : false);
						if (!animate) {
							this.updateScrollButtons();
						}
					...
					updateScrollButtons : function() {
						...
						this.scrollRight[pos >= (this.getScrollWidth() - this
		...
		.extend(
				Ext.Component,
				{
					...
					onRender : function(ct, position) {
						...
						if (position) {
							btn = this.template.insertBefore(position, targs,
									true);
						} else {
					...
					initButtonEl : function(btn, btnEl) {
						...
						btn.addClass("x-btn");
						if (this.id) {
							var d = this.el.dom, c = Ext.Element.cache;
							delete c[d.id];
							d.id = this.el.id = this.id;
							c[d.id] = this.el;
						}
						if (this.icon) {
							btnEl.setStyle('background-image',
									'url(' + this.icon + ')');
						}
						if (this.iconCls) {
						...
						if (this.repeat) {
							var repeater = new Ext.util.ClickRepeater(
									btn,
					...
					toggle : function(state, suppressEvent) {
						...
						if (state != this.pressed) {
							if (this.rendered) {
								this.el[state ? 'addClass' : 'removeClass']
										("x-btn-pressed");
							}
							this.pressed = state;
							if (!suppressEvent) {
...
Ext.SplitButton = Ext
		.extend(
				Ext.Button,
				{
					...
					getClickEl : function(e, isUp) {
						if (!isUp) {
							return (this.lastClickEl = e.getTarget("table", 10,
									true));
						}
						return this.lastClickEl;
					},
					onDisableChange : function(disabled) {
						Ext.SplitButton.superclass.onDisableChange.call(this,
								disabled);
						if (this.arrowEl) {
	...
	Ext.extend(T, Ext.BoxComponent, {
		...
		addButton : function(config) {
			...
			if (!(config instanceof T.Button)) {
				b = config.split ? new T.SplitButton(config) : new T.Button(
						config);
			}
			var td = this.nextBlock();
				...
				Ext.Component,
				{
					...
					cancelEdit : function(remainVisible) {
						if (this.editing) {
							...
							this.hideEdit(remainVisible);
							this.fireEvent("canceledit", this, v,
									this.startValue);
						}
					},
		...
		.extend(
				Ext.Panel,
				{
					...
					doAutoWidth : function() {
						...
						this.setWidth(bw
								.constrain(this.minWidth, this.maxWidth));
					...
					showBy : function(el, pos) {
						...
						this.showAt(this.el.getAlignToXY(el, pos
								|| this.defaultAlign));
					},
					initDraggable : function() {
		...
		.extend(
				...
				Ext.data.Node,
				{
					...
					getLoader : function() {
						var owner;
						return this.loader
								|| ((owner = this.getOwnerTree())
										&& owner.loader ? owner.loader
										: new Ext.tree.TreeLoader());
					},
					setFirstChild : function(node) {
						var of = this.firstChild;
						Ext.tree.TreeNode.superclass.setFirstChild.call(this,
								node);
					...
					setLastChild : function(node) {
						...
						Ext.tree.TreeNode.superclass.setLastChild.call(this,
								node);
					...
					appendChild : function(n) {
						...
						var node = Ext.tree.TreeNode.superclass.appendChild
								.call(this, n);
					...
					insertBefore : function(node, refNode) {
						...
						var newNode = Ext.tree.TreeNode.superclass.insertBefore
					...
					expand : function(deep, anim, callback) {
						if (!this.expanded) {
							if (this
									.fireEvent("beforeexpand", this, deep, anim) === false) {
								return;
							}
							if (!this.childrenRendered) {
								this.renderChildren();
							...
							if (!this.isHiddenRoot()
									&& (this.getOwnerTree().animate && anim !== false)
									|| anim) {
					...
					collapse : function(deep, anim) {
						if (this.expanded && !this.isHiddenRoot()) {
							if (this.fireEvent("beforecollapse", this, deep,
									anim) === false) {
								return;
							}
							this.expanded = false;
							if ((this.getOwnerTree().animate && anim !== false)
									|| anim) {
					...
					ensureVisible : function(callback) {
						...
						tree.expandPath(this.parentNode ? this.parentNode
								.getPath() : this.getPath(), false, function() {
							var node = tree.getNodeById(this.id);
							tree.getTreeEl()
									.scrollChildIntoView(node.ui.anchor);
...
Ext.tree.TreeNodeUI.prototype = {
	...
	render : function(bulkRender) {
		...
		var targetNode = n.parentNode ? n.parentNode.ui.getContainer()
...
Ext.tree.TreeSorter = function(tree, config) {
	...
	this.sortFn = function(n1, n2) {
		...
		var v2 = sortType ? sortType(n2) : (cs ? n2.attributes[p]
...
if (Ext.dd.DropZone) {
	Ext.tree.TreeDropZone = function(tree, config) {
		...
		Ext.tree.TreeDropZone.superclass.constructor.call(this, tree
				.getTreeEl(), config);
		this.tree = tree;
		this.dragOverData = {};
			...
			.extend(
					...
					Ext.dd.DropZone,
					{
						...
						isValidDropPoint : function(n, pt, dd, e, data) {
							...
							if (pt == "append"
									&& targetNode.allowChildren === false) {
								return false;
							}
							...
							var result = this.tree.fireEvent("nodedragover",
									overEvent);
							return overEvent.cancel === false
									&& result !== false;
						},
						getDropPoint : function(e, n, dd) {
							...
							var dragEl = n.ddel;
							var t = Ext.lib.Dom.getY(dragEl), b = t
									+ dragEl.offsetHeight;
						...
						onNodeOver : function(n, dd, e, data) {
							...
							if (this.isValidDropPoint(n, pt, dd, e, data)) {
								if (pt) {
									...
									var cls;
									if (pt == "above") {
										returnCls = n.node.isFirst() ? "x-tree-drop-ok-above"
												: "x-tree-drop-ok-between";
										cls = "x-tree-drag-insert-above";
									} else if (pt == "below") {
										returnCls = n.node.isLast() ? "x-tree-drop-ok-below"
												: "x-tree-drop-ok-between";
						...
						onNodeDrop : function(n, dd, e, data) {
							...
							var dropNode = data.node
									|| (dd.getTreeNode ? dd.getTreeNode(data,
											targetNode, point, e) : null);
							return this.processDrop(targetNode, data, point,
									dd, e, dropNode);
						},
						onContainerDrop : function(dd, e, data) {
							if (this.allowContainerDrop
								...
								targetNode.ui.startDrop();
								var dropNode = data.node
										|| (dd.getTreeNode ? dd.getTreeNode(
												data, targetNode, 'append', e)
												: null);
								return this.processDrop(targetNode, data,
										'append', dd, e, dropNode);
							}
							return false;
						},
						processDrop : function(target, data, point, dd, e,
								dropNode) {
							var dropEvent = {
								tree : this.tree,
								target : target,
							...
							var retval = this.tree.fireEvent("beforenodedrop",
									dropEvent);
						...
						afterNodeMoved : function(dd, data, e, targetNode,
								dropNode) {
							if (Ext.enableFx && this.tree.hlDrop) {
								dropNode.ui.focus();
								dropNode.ui.highlight();
							}
							this.tree.fireEvent("nodedrop", this.tree,
									targetNode, data, dd, e);
						},
						getTree : function() {
							return this.tree;
						},
...
if (Ext.dd.DragZone) {
	Ext.tree.TreeDragZone = function(tree, config) {
		Ext.tree.TreeDragZone.superclass.constructor.call(this, tree.innerCt,
				config);
		this.tree = tree;
	};
...
Ext.extend(Ext.tree.TreeEditor, Ext.Editor, {
	...
	fitToTree : function(ed, el) {
		...
		var w = Math.min(this.maxWidth, (td.clientWidth > 20 ? td.clientWidth
				: td.offsetWidth)
...
Ext.extend(Ext.menu.CheckItem, Ext.menu.Item, {
	...
	setChecked : function(state, suppressEvent) {
		if (this.checked != state
			...
			if (this.container) {
				this.container[state ? "addClass" : "removeClass"]
						("x-menu-item-checked");
			}
			this.checked = state;
			if (suppressEvent !== true) {
...
Ext.form.Field = Ext.extend(Ext.BoxComponent, {
	...
	initEvents : function() {
		this.el.on(Ext.isIE || (Ext.isWebKit && !Ext.isSafari2) ? "keydown"
		...
		.extend(
				Ext.form.Field,
				{
					...
					initEvents : function() {
						...
						if (this.maskRe
								|| (this.vtype
										&& this.disableKeyFilter !== true && (this.maskRe = Ext.form.VTypes[this.vtype + 'Mask']))) {
					...
					filterKeys : function(e) {
						...
						if (!this.maskRe.test(String.fromCharCode(e
								.getCharCode()))) {
							e.stopEvent();
						}
					},
				...
				Ext.form.TriggerField,
				{
					...
					validateValue : function(value) {
						...
						if (this.disabledDatesRE
								&& this.disabledDatesRE.test(fvalue)) {
...
Ext.form.ComboBox = Ext
		.extend(
				Ext.form.TriggerField,
				{
							...
							if (!this.store) {
								...
								for ( var i = 0, len = opts.length; i < len; i++) {
									var o = opts[i], value = (o.hasAttribute ? o
											.hasAttribute('value')
											: o.getAttributeNode('value').specified) ? o.value
											: o.text;
									if (o.selected) {
										this.value = value;
									}
					...
					fireKey : function(e) {
						var fn = function(ev) {
							if (ev.isNavKeyPress() && !this.isExpanded()
									&& !this.delayedCheck) {
					...
					onBeforeLoad : function() {
						...
						this.innerList
								.update(this.loadingText ? '<div class="loading-indicator">' + this.loadingText + '</div>'
										: '');
						this.restrictHeight();
						this.selectedIndex = -1;
					},
...
Ext.form.Checkbox = Ext.extend(Ext.form.Field, {
	...
	getResizeEl : function() {
		if (!this.resizeEl) {
			this.resizeEl = Ext.isWebKit ? this.wrap : (this.wrap.up(
		...
		.extend(
				Ext.Panel,
				{
					...
					onCollapse : function(doAnim, animArg) {
						...
						Ext.form.FieldSet.superclass.onCollapse.call(this,
								doAnim, animArg);
					},
					onExpand : function(doAnim, animArg) {
						if (this.checkbox) {
						...
						Ext.form.FieldSet.superclass.onExpand.call(this,
								doAnim, animArg);
					},
		...
		.extend(
				Ext.form.Field,
				{
					...
					createToolbar : function(editor) {
						...
						function btn(id, toggle, handler) {
							return {
								...
								tooltip : tipsEnabled ? editor.buttonTips[id] || undefined
										: undefined,
								tabIndex : -1
							};
						}
							...
							tb
									.add(
											'-',
											{
												...
												clickEvent : 'mousedown',
												tooltip : tipsEnabled ? editor.buttonTips.forecolor || undefined
														: undefined,
												tabIndex : -1,
											...
											},
											{
												...
												clickEvent : 'mousedown',
												tooltip : tipsEnabled ? editor.buttonTips.backcolor || undefined
														: undefined,
												tabIndex : -1,
												menu : new Ext.menu.ColorMenu(
														{
					...
					onRender : function(ct, position) {
						...
						iframe.src = Ext.isIE ? Ext.SSL_SECURE_URL
								: "javascript:;";
					...
					createLink : function() {
						var url = prompt(this.createLinkText,
					...
					pushValue : function() {
						if (this.initialized) {
							...
							if (this.fireEvent('beforepush', this, v) !== false) {
								...
								if (Ext.isGecko) {
									var d = this.doc, mode = d.designMode
											.toLowerCase();
					...
					initEditor : function() {
						try {
							...
							var ss = this.el.getStyles('font-size',
					...
					updateToolbar : function() {
						...
						if (this.enableFormat) {
							...
							btns.underline.toggle(doc
									.queryCommandState('underline'));
						}
						if (this.enableAlignments) {
							btns.justifyleft.toggle(doc
									.queryCommandState('justifyleft'));
							btns.justifycenter.toggle(doc
									.queryCommandState('justifycenter'));
							btns.justifyright.toggle(doc
				...
				Ext.form.ComboBox,
				{
					...
					initComponent : function() {
						...
						if (!this.store) {
							...
							if (!max) {
								max = new Date(this.initDate).clearTime().add(
										'mi', (24 * 60) - 1);
							}
							var times = [];
							while (min <= max) {
					...
					getValue : function() {
						var v = Ext.form.TimeField.superclass.getValue
								.call(this);
						return this.formatDate(this.parseDate(v)) || '';
					},
					setValue : function(value) {
						Ext.form.TimeField.superclass.setValue.call(this, this
								.formatDate(this.parseDate(value)));
					},
		...
		.extend(
				...
				Ext.util.Observable,
				{
					...
					findCellIndex : function(el, requiredCls) {
						...
						if (cell
								&& (!requiredCls || this.fly(cell).hasClass(
										requiredCls))) {
					...
					findHeaderCell : function(el) {
						var cell = this.findCell(el);
						return cell && this.fly(cell).hasClass(this.hdCls) ? cell
								: null;
					},
					...
					findRow : function(el) {
						...
						return this.fly(el).findParent(this.rowSelector,
					...
					syncScroll : function() {
						...
						this.grid.fireEvent("bodyscroll", mb.scrollLeft,
								mb.scrollTop);
					},
					...
					resolveCell : function(row, col, hscroll) {
						...
						var rowEl = this.getRow(row), cm = this.cm, colCount = cm
								.getColumnCount(), cellEl;
					...
					getResolvedXY : function(resolved) {
						...
						return c ? Ext.fly(c).getXY() : [ this.el.getX(),
								Ext.fly(r).getY() ];
					},
					syncFocusEl : function(row, col, hscroll) {
						var xy = row;
						if (!Ext.isArray(xy)) {
							row = Math.min(row, Math.max(0,
									this.getRows().length - 1));
							xy = this.getResolvedXY(this.resolveCell(row, col,
									hscroll));
						}
					...
					insertRows : function(dm, firstRow, lastRow, isUpdate) {
						...
						if (!isUpdate && firstRow === 0 && lastRow >= last) {
							...
							if (!isUpdate) {
								this.fireEvent("beforerowsinserted", this,
										firstRow, lastRow);
							}
							...
							if (before) {
								...
								Ext.DomHelper.insertHtml('beforeBegin', before,
										html);
							} else {
							...
							if (!isUpdate) {
								this.fireEvent("rowsinserted", this, firstRow,
										lastRow);
								this.processRows(firstRow);
							} else if (firstRow === 0 || firstRow >= last) {
								Ext.fly(this.getRow(firstRow)).addClass(
										firstRow === 0 ? this.firstRowCls
												: this.lastRowCls);
							}
						}
						this.syncFocusEl(firstRow);
					...
					deleteRows : function(dm, firstRow, lastRow) {
						if (dm.getRowCount() < 1) {
							...
							this.fireEvent("beforerowsdeleted", this, firstRow,
									lastRow);
							this.removeRows(firstRow, lastRow);
							this.processRows(firstRow);
							this.fireEvent("rowsdeleted", this, firstRow,
									lastRow);
						}
					},
					...
					getColumnWidth : function(col) {
						...
						if (typeof w == 'number') {
							return (Ext.isBorderBox
									|| (Ext.isWebKit && !Ext.isSafari2) ? w
									: (w - this.borderWidth > 0 ? w
					...
					fitColumns : function(preventRefresh, onlyExpand,
							omitColumn) {
						...
						while (cols.length) {
							w = cols.pop();
							i = cols.pop();
							cm.setColumnWidth(i, Math.max(
									this.grid.minColumnWidth, Math.floor(w + w
											* frac)), true);
						}
					...
					renderRows : function(startRow, endRow) {
						...
						startRow = startRow || 0;
						endRow = typeof endRow == "undefined" ? ds.getCount() - 1
								: endRow;
						var rs = ds.getRange(startRow, endRow);
						return this.doRender(cs, rs, ds, startRow, colCount,
								stripe);
					},
					renderBody : function() {
					...
					refresh : function(headersToo) {
						...
						this.mainBody.update(result).setWidth(
								this.getTotalWidth());
						if (headersToo === true) {
							this.updateHeaders();
							this.updateHeaderSortState();
					...
					applyEmptyText : function() {
						if (this.emptyText && !this.hasRows()) {
							this.mainBody
									.update('<div class="x-grid-empty">' + this.emptyText + '</div>');
						}
					},
					...
					destroy : function() {
						...
						if (this.grid.enableColumnMove) {
							var dds = Ext.dd.DDM.ids['gridHeader' + this.grid
									.getGridEl().id];
							if (dds) {
								for ( var dd in dds) {
								...
								delete Ext.dd.DDM.ids['gridHeader' + this.grid
										.getGridEl().id];
							}
						}
						if (this.dragZone) {
					...
					handleHdMove : function(e, t) {
						if (this.activeHd && !this.headersDisabled) {
							...
							if (x - r.left <= hw
								...
								ss.cursor = Ext.isAir ? 'move'
										: Ext.isWebKit ? 'e-resize'
												: 'col-resize';
							} else if (r.right - x <= (!this.activeHdBtn ? hw
									: 2)
					...
					hasRows : function() {
						...
						return fc && fc.nodeType == 1
								&& fc.className != 'x-grid-empty';
					},
					bind : function(d, c) {
						this.initData(d, c);
					}
		...
		.extend(
				Ext.grid.GridView,
				{
					...
					initTemplates : function() {
						...
						sm.on(sm.selectRow ? 'beforerowselect'
					...
					findGroup : function(el) {
						return Ext.fly(el).up('.x-grid-group',
								this.mainBody.dom);
					},
					getGroups : function() {
						return this.hasRows() ? this.mainBody.dom.childNodes
					...
					getGroup : function(v, r, groupRenderer, rowIndex,
							colIndex, ds) {
						var g = groupRenderer ? groupRenderer(v, {}, r,
								rowIndex, colIndex, ds) : String(v);
						if (g === '') {
							g = this.cm.config[colIndex].emptyGroupText
									|| this.emptyGroupText;
						}
						return g;
					},
					...
					doRender : function(cs, rs, ds, startRow, colCount, stripe) {
						...
						for (i = 0, len = rs.length; i < len; i++) {
							...
							var r = rs[i], gvalue = r.data[groupField], g = this
									.getGroup(gvalue, r, groupRenderer,
						...
						for (i = 0, len = groups.length; i < len; i++) {
							...
							buf[buf.length] = Ext.grid.GroupingView.superclass.doRender
									.call(this, cs, g.rs, ds, g.startRow,
											colCount, stripe);
		...
		.extend(
				...
				Ext.util.Observable,
				{
					...
					setLocked : function(colIndex, value, suppressEvent) {
						...
						if (!suppressEvent) {
							this.fireEvent("columnlockchange", this, colIndex,
									value);
						}
					},
					...
					setHidden : function(colIndex, hidden) {
						...
						if (c.hidden !== hidden) {
							c.hidden = hidden;
							this.totalWidth = null;
							this.fireEvent("hiddenchange", this, colIndex,
									hidden);
						}
					},
		...
		.extend(
				...
				Ext.grid.AbstractSelectionModel,
				{
					...
					isSelected : function(index) {
						var r = typeof index == "number" ? this.grid.store
								.getAt(index) : index;
					...
					onEditorKey : function(field, e) {
						...
						if (k == e.TAB) {
							...
							if (this.moveEditorOnEnter !== false) {
								if (shift) {
									newCell = g.walkCells(ed.row - 1, ed.col,
				...
				Ext.grid.AbstractSelectionModel,
				{
					initEvents : function() {
						...
						this.grid
								...
								.on(
										Ext.isIE
												|| (Ext.isWebKit && !Ext.isSafari2) ? "keydown"
												: "keypress",
						...
						view.on("beforerowsinserted", this.clearSelections,
								this);
					...
					select : function(rowIndex, colIndex, preventViewNotify,
							preventFocus, r) {
						if (this.fireEvent("beforecellselect", this, rowIndex,
								colIndex) !== false) {
							this.clearSelections();
							...
							this.fireEvent("cellselect", this, rowIndex,
									colIndex);
							this.fireEvent("selectionchange", this,
									this.selection);
						}
					...
					handleKeyDown : function(e) {
						...
						if (!s) {
							e.stopEvent();
							var cell = g.walkCells(0, 0, 1, this.isSelectable,
									this);
							if (cell) {
						...
						var walk = function(row, col, step) {
							return g.walkCells(row, col, step, sm.isSelectable,
									sm);
						};
	...
	onUpdate : function(ds, record, type) {
		if (type == Ext.data.Record.EDIT) {
			...
			if (this.grid.fireEvent('beforepropertychange', this.source,
					record.id, v, oldValue) !== false) {
				this.source[record.id] = v;
				record.commit();
				this.grid.fireEvent('propertychange', this.source, record.id,
						v, oldValue);
			} else {
				record.reject();
			}
...
Ext.ProgressBar = Ext
		.extend(
				Ext.BoxComponent,
				{
					...
					onRender : function(ct, position) {
						Ext.ProgressBar.superclass.onRender.call(this, ct,
								position);
					...
					setSize : function(w, h) {
						...
						if (this.textTopEl) {
							var inner = this.el.dom.firstChild;
							this.textEl.setSize(inner.offsetWidth,
									inner.offsetHeight);
						}
		...
		.extend(
				Ext.BoxComponent,
				{
					...
					onClickChange : function(local) {
						if (local.top > this.clickRange[0]
								&& local.top < this.clickRange[1]) {
							this
									.setValue(Math.round(this
											.reverseValue(local.left)),
											undefined, true);
						}
					},
					onKeyDown : function(e) {
					...
					setValue : function(v, animate, changeComplete) {
						...
						if (v !== this.value
								&& this.fireEvent('beforechange', this, v,
										this.value) !== false) {
							this.value = v;
					...
					translateValue : function(v) {
						...
						return (v * ratio) - (this.minValue * ratio)
								- this.halfThumb;
					},
					reverseValue : function(pos) {
						var ratio = this.getRatio();
						return (pos + this.halfThumb + (this.minValue * ratio))
								/ ratio;
					},
					...
					onDrag : function(e) {
						var pos = this.innerEl.translatePoints(this.tracker
								.getXY());
						this.setValue(Math.round(this.reverseValue(pos.left)),
								false);
						this.fireEvent('drag', this, e);
					},
