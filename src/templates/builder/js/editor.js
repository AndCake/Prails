...
var Editor = (function(){
  ...
  Editor.prototype = {
    ...
    jumpToLine: function(line, offset) {
      if (!offset) offset = 0;
      if (line <= 1 || !this.container.firstChild) {
        ...
        while (true) {
          ...
          if (line == 2 && pos.nodeName != "BR") offset--;
          if ((line <= 1 && offset <= 1) || !pos.nextSibling) break;
          pos = pos.nextSibling;
        }
    ...
    currentLine: function(withOffset) {
      var pos = select.cursorPos(this.container, true), line = 1, offset = 1;
      if (!pos) return 1;
      for (cursor = pos.node; cursor; cursor = cursor.previousSibling)
        if (cursor.nodeName != "BR" && line == 1) offset++; else if (cursor.nodeName == "BR") line++;
      if (withOffset == true) return [line, offset]; else return line;
    },
