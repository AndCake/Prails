...
var CodeMirror = (function(){
  ...
  function CodeMirror(place, options) {
    ...
    this.originalValue = options.content;
    if (this.options.crc32 != null && this.options.crc32Id.length > 0 && this.options.mergeUrl.length > 0) {
    	Builder.registerUpdater(this.options.crc32Id, function(event){
    		var crc = event.crc32;
    		if (event.crc32 != this.options.crc32) {
				new Ajax.Request(this.options.mergeUrl, {
					method: "post",
					parameters: {
						oldDB: this.originalValue,
						newUser: this.getCode()
					},
					onSuccess: function(req) {
						var cl = this.currentLine();
						var oldAndNew = req.responseText.split("\n6c7f3ed76b9e883ec951f60dedb25491\n");
						this.setCode(oldAndNew[1]);
						this.jumpToLine(cl);
						this.originalValue = oldAndNew[0];
						this.options.crc32 = crc; 
					}.bind(this)
				});
       		}
    	}.bind(this));
